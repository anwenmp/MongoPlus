...
已执行 sensitive-word 定向回归，当前 9 个转换器级测试覆盖 FieldHandler order、最新值传递、旧实现兼容、TypeHandler→Encrypt 与 LOCAL 组合。core、Boot 2/3 starter、Boot 4 starter 定向 compile 已通过；reactor 外 `mongo-plus-test` 当前 24 项测试全部通过，其中新增 3 项覆盖注解 privateKey 传递及 RSA/SM2 全局 privateKey 回退。未执行或未形成已验证结论：全 reactor `test/package/verify/deploy`、独立 BOM、真实 MongoDB 集成测试、Java 8/多 JDK 加密 provider 矩阵，以及 Boot 2、Boot 3、Boot 4、Solon 启动测试。
