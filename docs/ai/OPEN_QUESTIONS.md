...
> 收口日期：2026-08-08。每个问题只归入一类。“已确认缺陷”要求当前源码控制流闭合；“高风险设计”不是缺陷结论；“运行验证”不得在测试前写成必现外部行为。已解决观察不再保留。

## A. 已确认缺陷（1 个部分修复，9 个已修复）
...
9. **已修复：Backup 多页 JSON 尾逗号：** 2026-08-07 原实现以跨页累计的 `count < totalDocuments` 决定分页 entry 内的逗号，非末页最后一条后会留下逗号。现改为按当前页局部 `firstDocument` 决定文档前分隔符；回归位于 `../../mongo-plus-test/src/test/java/com/mongoplus/manager/BackupManagerPaginationTest.java`，覆盖 3/2、2/2、1/2、4/2、5/2 和空集合。实际以当前 core classes 单独运行 JUnitCore，6 项通过；当时独立 Maven test 被无关的 `business/OptimisticLockerInterceptorTest` 源码编译错误阻断，2026-08-08 当前工作树全量测试已通过。恢复只处理首个 entry、导出/恢复编码不一致等问题仍未解决。[Backup / Restore](features/BACKUP_AND_RESTORE.md)
10. **部分修复：Recorder 上下文、namespace 与递归保护缺陷：** 2026-08-08 已在审计保存的 finally 中恢复进入前的 datasource（原来未设置则 clear）并移除 `OperationResult`，保持保存异常传播；独立 `mongo-plus-test` 6 项定向回归与当前全量 41 项测试通过。仍未修复：单槽 ThreadLocal 的嵌套覆盖、动态 collection 后记录原 namespace，以及用户覆盖忽略列表破坏默认递归保护。[Data Change Recorder](features/DATA_CHANGE_RECORDER.md)
