...
|---|---|---|---|---|---|---|---|
| NOT 与 EXPR 共用 `Filters.expr` | 待验证 | `BuildCondition` 的 `case NOT` 与 `case EXPR` 连续落入同一分支并调用 `Filters.expr` | 尚未对公开 `not(...)` 与 `expr(...)` 的最终 BSON/命中集做对照 | 构造单键和嵌套条件，断言 BSON 并在 MongoDB 查询 | `architecture/QUERY_WRAPPER.md`、`TESTING.md` | `mongo-plus-core/.../handlers/condition/BuildCondition.java:142-150` | core、所有 starter/plugin |
| NOT 多键只读取第一个键 | 待验证 | 分支对内部 `BasicDBObject.keySet().stream().findFirst()`，仅包装首键 | 键顺序与公开 API允许的多键语义尚未实测 | 多键 NOT 重复执行，比较序列化 BSON、顺序和命中集 | `architecture/QUERY_WRAPPER.md` | `BuildCondition.java:144-150` | core |
...
| `regex/like(null)` | 待验证 | 构建阶段直接调用 `conditionMetaObject.getValue().toString()`，null 可触发 NPE；当前工作区已有公开 `regex/like` 测试源码断言构建期 NPE | 测试模块未进入根 reactor，且本次独立执行因缺少本地 core 构件失败；变体也未全部覆盖 | 先使现有测试可执行，再补 `likeLeft/likeRight` 与 Lambda 变体并断言构建时机和异常类型 | `architecture/QUERY_WRAPPER.md` | `BuildCondition.java:99-106`；`BuildConditionRegexTest.java` | core |
