...
未跟踪、reactor 外的 `mongo-plus-test` 当前有两个 JUnit 4 测试源：`BuildConditionRegexTest` 和 `BuildConditionNotTest`，并存在本机 surefire 输出。它们的目录状态可变化，不能当成仓库发布门禁。真实 MongoDB、Boot/Solon context、事务和分片均没有 reactor 内自动化保障。
